...
export function Header({ breadcrumbs = [] }) {
    ...
    return (
        <header className="sticky top-0 z-30 flex h-16 items-center border-b border-zinc-800/60 bg-zinc-950/60 px-6 backdrop-blur-xl">
            <nav className="flex flex-1 items-center gap-1 text-sm">
                {allBreadcrumbs.map((crumb, index) => (
                    <span key={index} className="flex items-center gap-1">
                        {index > 0 && <ChevronRight className="h-3.5 w-3.5 text-zinc-600" />}
                        {index === 0 ? <Home className="h-3.5 w-3.5 text-zinc-500" /> : null}
                        <span className={cn("transition-colors", index === allBreadcrumbs.length - 1 ? "text-zinc-200" : "text-zinc-500 hover:text-zinc-300")}>
                            {crumb.label}
                        </span>
                    </span>
                ))}
            ...
            <div className="flex items-center gap-3">
                ...
                <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        <button className="flex items-center gap-2.5 rounded-xl py-1.5 pl-1.5 pr-3 transition-colors hover:bg-zinc-800/60">
                            ...
                            <div className="hidden text-left md:block">
                                <p className="text-sm font-medium text-zinc-200">{user?.name || "Guest"}</p>
