                            ...
                            <TableBody>
                                {rows.map((s) => (
                                    <TableRow key={s.regNo}>
                                        ...
                                        <TableCell className="text-sm text-zinc-300">{s.aptitudeCount + s.codingCount}</TableCell>
                                        <TableCell className="text-sm text-zinc-300">{s.aptitudeCount ? `${s.aptitudeAverage}%` : "—"}</TableCell>
                                        <TableCell className="text-sm text-zinc-300">{s.codingCount ? `${s.codingAverage}%` : "—"}</TableCell>
                                        <TableCell className="text-sm text-zinc-300">
                                            {s.lastInterview ? <span className="text-amber-400">★ {s.lastInterview.rating}/5</span> : "—"}
                                        </TableCell>
                                        <TableCell>
                                            {s.hasResume ? (
                                                <span className="flex items-center gap-1 text-xs text-violet-400"><FileText className="h-3.5 w-3.5" /> {s.topCategory || s.categories[0] || "Parsed"}</span>
                                            ) : (
