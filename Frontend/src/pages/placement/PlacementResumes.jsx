                                ...
                                {resumes.map((r) => (
                                    <TableRow key={r.id}>
                                        <TableCell>
                                            ...
                                            <p className="text-xs text-zinc-500">{r.regNo}</p>
                                        </TableCell>
                                        <TableCell className="text-sm text-zinc-400">{r.fileName}</TableCell>
                                        <TableCell>
                                            <div className="flex max-w-xs flex-wrap gap-1">
                                                {(r.skills || []).slice(0, 4).map((s) => (
                                                    <span key={s} className="rounded bg-zinc-800 px-1.5 py-0.5 text-xs text-zinc-300">{s}</span>
                                                ))}
                                                {(r.skills || []).length > 4 && <span className="text-xs text-zinc-500">+{(r.skills || []).length - 4}</span>}
                                            </div>
                                        </TableCell>
                                        <TableCell className="text-sm text-zinc-300">{(r.categories || []).map((c) => (typeof c === "string" ? c : c.name)).join(", ") || "—"}</TableCell>
                                        <TableCell className="text-sm text-violet-400">{r.topCategory || "—"}</TableCell>
                                        <TableCell className="text-right">
                                            <div className="flex items-center justify-end gap-1">
                                                ...
                                                <Button variant="ghost" size="icon" className="h-8 w-8 text-red-400 hover:text-red-300" onClick={() => remove(r)}><Trash2 className="h-4 w-4" /></Button>
                                            </div>
                                        </TableCell>
                                    </TableRow>
                                ))}
